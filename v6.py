...
def adaptive_water_logic(comp_name, sensor_value, current_time):
    """
    Adaptive water logic:
      - If sensor_value is below 450, trigger watering (set state True and water for 5 seconds).
      - Once triggered (state True), continue watering (extend watering for 5 seconds)
        as long as sensor_value is below 680.
    ...
    if sensor_value < 450:
        ...
        # If already watering, extend watering period
        if comp["state"]:
            comp["watering_until"] = current_time + 5
        # If not already watering, do nothing (waiting for a drop below 450)
    else:
        # Sensor reading is >=680: stop watering.
        comp["state"] = False
        comp["watering_until"] = 0
