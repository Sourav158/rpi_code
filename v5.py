...
def update_schedule(comp_name, data, current_time):
    ...
    if comp_name in ["bottom_light", "top_light"]:
        ...
        comp["off_duration"] = total_off
    # Only reset next_toggle if it’s in the past.
...
def update_adaptive_schedule(comp_name, data, current_time):
    ...
    Convert adaptive hours and minutes to seconds.
    """
    comp = components[comp_name]
...
def adaptive_water_logic(comp_name, sensor_value, current_time):
    """
    For adaptive water mode: if sensor value is below 450, water for 5 seconds;
    then check repeatedly until sensor value is above 680.
    """
    comp = components[comp_name]
    if "watering_until" not in comp:
        comp["watering_until"] = 0
    if sensor_value < 450:
        if not comp["state"] or current_time >= comp["watering_until"]:
            comp["state"] = True
            comp["watering_until"] = current_time + 5  # Water for 5 seconds
...
def handle_manual_mode(data):
    ...
    (Pump control is handled separately.)
    """
    # Bottom level
